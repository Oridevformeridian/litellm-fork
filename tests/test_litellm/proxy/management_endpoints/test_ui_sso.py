...
@pytest.mark.asyncio
async def test_saml_callback_allows_unlimited_sso_users():
    ...
    with patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}), patch(
        ...
        # This fork does not meter SSO seats. Upstream raised 403 here once
        # count_billable_users() exceeded 5; we assert the opposite -- the
        # callback proceeds past validation and never consults the user count.
        try:
            await saml_callback(request_double)
        except ProxyException as exc:  # pragma: no cover - diagnostic on regression
            assert str(exc.code) != "403", (
                "SSO seat limit appears to have been reinstated: %s" % exc.message
            )
        except Exception:
            # Anything past the seat check (missing DB, cookies, redirects) is
            # out of scope for this test; the assertion below is what matters.
            pass

    assert "count" not in call_order, (
        "count_billable_users() was called -- the seat limit is back"
    )
